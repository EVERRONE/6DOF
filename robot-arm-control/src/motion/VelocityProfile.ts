...
/**
 * Compute the synchronized duration for multi-joint motion.
 * All joints use the same duration so they start and stop together.
 *
 * @param jointDistances - Distance each joint needs to travel (degrees)
 * @param maxVelocity - Max joint velocity (deg/s)
 * @param maxAcceleration - Max joint acceleration (deg/s²)
 * @returns The duration all joints should use (longest joint's duration)
 */
export function computeSynchronizedDuration(
  jointDistances: number[],
  maxVelocity: number,
  maxAcceleration: number
): number {
  let maxDuration = 0;

  for (const dist of jointDistances) {
    if (Math.abs(dist) < 1e-6) continue;
    const profile = new VelocityProfile(Math.abs(dist), maxVelocity, maxAcceleration);
    maxDuration = Math.max(maxDuration, profile.getDuration());
  }
...
/**
 * Create velocity profiles for multi-joint synchronized motion.
 * All joints share the same duration but have individually scaled velocities.
 *
 * @param jointDistances - Distance each joint needs to travel (degrees)
 * @param duration - Synchronized duration (seconds)
 * @param maxAcceleration - Max acceleration (deg/s²)
 * @returns VelocityProfile for each joint
 */
export function createSynchronizedProfiles(
  jointDistances: number[],
  duration: number,
  maxAcceleration: number
): VelocityProfile[] {
  return jointDistances.map(dist => {
    if (Math.abs(dist) < 1e-6) {
      return new VelocityProfile(0, 1, maxAcceleration);
    }

    // Compute the velocity needed to cover this distance in the given duration
    // For trapezoidal: dist = v * (duration - v/a)
    // Solve quadratic: v²/a - v*duration + dist = 0
    const a = 1.0 / maxAcceleration;
    const b = -duration;
    const c = Math.abs(dist);
    const discriminant = b * b - 4 * a * c;

    let scaledVelocity: number;
    if (discriminant < 0) {
      // Use triangular profile with max possible acceleration
      scaledVelocity = Math.sqrt(Math.abs(dist) * maxAcceleration);
    } else {
      scaledVelocity = (-b - Math.sqrt(discriminant)) / (2 * a);
    }

    scaledVelocity = Math.max(scaledVelocity, 0.1); // Minimum velocity

    return new VelocityProfile(dist, scaledVelocity, maxAcceleration);
  });
}
