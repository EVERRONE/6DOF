...
It was audited against code on 2026-03-21.

## Audit Status

- Web tests verified: `npm test -- --watchAll=false --runInBand`
  - Result: 19 suites passed, 52 tests passed
...
- `stl_meshes/`
  - Root mesh sources
- `CLAUDE.md`
  - Existing repo guide, useful but partially stale
...
- Runtime FK goes through `ForwardKinematics.solve()`, which now delegates to `UrdfChainKinematics`.
...
| --- | ---: | ---: | --- | --- | --- |
| J1 | -40 | 30 | no | n/a | false |
| J2 | -60 | 0 | yes | false | true |
| J3 | 0 | 70 | yes | true | true |
| J4 | -274 | 0 | yes | false | false |
| J5 | -280 | 0 | yes | false | false |
...
- drives `StepperController.setStreamingTargetAngles()` at 1 kHz
- tracks diagnostics:
  - tick jitter
...
- `CLAUDE.md`
  - useful overview, but some concrete facts are stale
  - example: it says J4/J5 are direction-inverted, but `config.h` currently has J2/J3 inverted
  - it mentions `public/models/robot.urdf`, which is not present in this repo
