 ...
 *
 * Phase 1+2 deliberately contains no command that can move the arm.
 * `stop` can only ever reduce motion, so it ships alongside the read-only set.
 */
export const COMMAND_TYPES = Object.freeze(['get_status', 'preview_move', 'stop']);

/** Commands that are refused unless the executor reports itself armed. */
export const COMMANDS_REQUIRING_ARM = Object.freeze([]);
